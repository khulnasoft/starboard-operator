...
2. Get the source code:
   ...
   ```
   $ git clone git@github.com:khulnasoft/starboard-operator.git
...
1. Build Docker images:
   ...
   This will build the `docker.io/khulnasoft/starboard-operator:dev` as well as `docker.io/khulnasoft/starboard-scanner-khulnasoft:dev`
   images. The second image is only used when you enable the Khulnasoft CSP scanner. By default Trivy is used as vulnerability
   scanner by pulling its official image accessible from DockerHub (`docker.io/khulnasoft/trivy:$TRIVY_VERSION`).
...
3. Send the definition of the VulnerabilityReport custom resource to the Kubernetes API:
   ...
   ```
   $ kubectl apply -f https://raw.githubusercontent.com/khulnasoft/starboard /master/kube/crd/vulnerabilityreports-crd.yaml
   ```
