...
var _ = Describe("The Khulnasoft API client", func() {
	...
	BeforeEach(func() {
		server = NewServer()
		khulnasoftClient = client.NewClient(server.URL(), client.Authorization{
	...
	Describe("fetching registries", func() {
		...
		Context("when the request succeeds", func() {
			...
			It("should make a request to fetch registries", func() {
				registries, err := khulnasoftClient.Registries().List()
		...
		Context("when the response is unauthorized", func() {
			...
			It("should return error", func() {
				_, err := khulnasoftClient.Registries().List()
	...
	Describe("fetching vulnerabilities", func() {
		...
		Context("when the request succeeds", func() {
			...
			It("should make a request to fetch vulnerabilities", func() {
				vulnerabilities, err := khulnasoftClient.Images().Vulnerabilities("Harbor", "library/nginx", "1.17")
				Expect(err).ToNot(HaveOccurred())
		...
		Context("when the request is unauthorized", func() {
			...
			It("should return error", func() {
				_, err := khulnasoftClient.Images().Vulnerabilities("Harbor", "library/nginx", "1.17")
				Expect(err).To(MatchError(client.ErrUnauthorized))
				Expect(server.ReceivedRequests()).To(HaveLen(1))
			})
		})
